...
TEST(ApproxLZ77ParCompressorTests, CompressEmptyString) {
    std::stringstream ss_in, ss_out, ss_out2;
    ss_in << "";
    InStreamView sv_in(ss_in);
    ApproxLZ77Encoder encoder(ss_out, sv_in.size());

    ApproxLZ77ParCompressor::Instance().compress(sv_in, encoder);
    ss_out.clear();
    ss_out.seekg(0);

    ApproxLZ77Decoder decoder2(ss_out);
    OutStreamView sv_out2(ss_out2);
    ApproxLZ77ParCompressor::Instance().decompress(decoder2, sv_out2);
    EXPECT_TRUE(ss_in.str().compare(ss_out2.str())==0);
}

TEST(ApproxLZ77ParCompressorTests, CompressText) {
    std::stringstream ss_in, ss_out, ss_out2;
